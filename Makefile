...
create-migration: ### make create-migration name=init_db создает миграцию для бд маркировки
	cd migrations/data_base && goose create $(name) sql && cd ../..
name ?= default_name

migrate-up:
	cd migrations/data_base && goose postgres "user=$(PG_USER) password=$(PG_PASSWORD) dbname=$(PG_DBNAME) host=$(PG_HOST) port=$(PG_PORT) sslmode=disable" up && cd ../..
