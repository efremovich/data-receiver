...
func (repo *costRepoImpl) SelectByCardIDAndDate(ctx context.Context, cardID int64, date time.Time) (*entity.Cost, error) {
	...
	query := `SELECT
						...
						FROM
							shop.costs
						WHERE
							card_id = $1 and created_at = $2;`

	err := repo.getReadConnection().Get(&result, query, cardID, date)
	if err != nil && errors.Is(err, sql.ErrNoRows) {
		return nil, ErrObjectNotFound
	} else if err != nil {
		return nil, fmt.Errorf("ошибка при поиске данные по cardID %d и дате %s", cardID, date.Format("2016-02-01"))
	}
